...
AI_REQUEST_TIMEOUT = 15     # Seconds per AI call
AI_MAX_TOKENS = 300
AI_TEMPERATURE = 0.85
...
async def init_db_pool():
    global db_pool
    db_pool = await asyncpg.create_pool(DATABASE_URL, min_size=2, max_size=20)  # Increased max_size
...
# ============== HELPER FUNCTIONS ==============
def utc_now():
    """Returns current datetime in UTC (naive)."""
...
def get_indian_time():
    """Returns current datetime in Indian timezone (IST, UTC+5:30) – aware."""
...
async def init_db():
    ...
    async with pool.acquire() as conn:
        ...
        if 'plan_expiry' not in col_names:
            await conn.execute("ALTER TABLE users ADD COLUMN plan_expiry TIMESTAMP")  # UTC naive
        ...
        # Add new columns for key health tracking
...
async def reset_daily_if_needed(user_id: int):
    pool = await get_db()
    today_utc = utc_now().date()  # UTC date
    async with pool.acquire() as conn:
...
async def can_send_message(user_id: int) -> (tuple):
    ...
    logger.info(f"User {user_id} plan: {plan['plan_type']}, count: {plan['daily_msg_count']}, limit: {limit}, expiry: {plan['plan_expiry']}")
...
async def check_rate_limit(user_id: int) -> bool:
    """Returns True if under limit, False if should be blocked."""
    pool = await get_db()
    now = datetime.now(timezone.utc)  # aware for timestamptz
    async with pool.acquire() as conn:
        # Get current window
        row = await conn.fetchrow("SELECT window_start, count FROM rate_limits WHERE user_id=$1", user_id)
        if not row:
            # First emessage, create new window
            await conn.execute(
...
async def giveplan_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user_exists:
            try:
                # Convert expiry to IST for display
                expiry_utc = utc_now() + timedelta(days=expiry_days)
...
# ============== AI CALL WITH FALLBACK & HEALTH TRACKING ==============
...
async def call_ai_with_fallback(messages, nickname):
    """Try all active API keys from database with health tracking."""
    pool = await get_db()
    now = datetime.now(timezone.utc)
    async with pool.acquire() as conn:
        # Fetch active keys that are not temporarily disabled
        rows = await conn.fetch("""
    ...
    for row in rows:
        ...
        try:
            ...
            # Success: reset error count and disabled_until
            async with pool.acquire() as conn:
                await conn.execute(
...
async def _record_key_failure(key_id: int):
    """Increment error count and disable key if too many failures."""
    pool = await get_db()
    async with pool.acquire() as conn:
        ...
        # If error_count >= 3, disable for 5 minutes
        await conn.execute("""
            UPDATE api_keys
...
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    expiry_utc = plan['plan_expiry']  # naive UTC
