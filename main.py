...
async def chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u.id in COLLECTING_MODE:
        ...
        # === PIC COLLECTION WITH ADMIN CHECK AND ERROR HANDLING ===
        if mode == "pic":
            # Verify admin status
            if not await is_admin(u.id):
                COLLECTING_MODE.pop(u.id, None)
                await msg.reply_text("❌ You are no longer admin. Collecting mode cancelled.")
                return
            ...
            if file_id:
                try:
                    ...
                    logger.error(f"Error adding pic: {e}")
                    await msg.reply_text("❌ Error adding pic. Try again later.")
            return

        # === STICKER COLLECTION WITH ADMIN CHECK AND ERROR HANDLING ===
        if mode == "sticker" and msg.sticker:
            ...
            try:
                ...
                await msg.reply_text("❌ Error adding sticker. Try again later.")
            return
