...
async def giveplan_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) != 2:
        await update.message.reply_text("Usage: /giveplan <user_id> <plan>")
        return
    try:
        ...
        await update.message.reply_text(f"✅ Plan {plan} given to user {target_id} for {expiry_days} days.")
        # Notify user
        try:
            await context.bot.send_message(
                chat_id=target_id,
                text=f"🎉 Congratulations Baby ❤️\n\nYour {plan.capitalize()} Plan is activated!\n\nDaily limit: {get_daily_limit(plan)} messages\nValid for: {expiry_days} days\n\nNow we can chat more together 🥺"
            )
        except Exception as e:
...
async def call_ai_with_fallback(messages, nickname):
    ...
    try:
        ...
        logger.warning("OpenRouter failed, trying Pollinations")
